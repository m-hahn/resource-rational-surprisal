data = read.csv("output/extractSlopes_All.py.tsv", sep="\t")
...
plot = ggplot(predictions, aes(x=EmbeddingBias, y=Surprisal, group=Condition, linetype=as.character(compatible.C), color=as.character(RC))) + geom_smooth(method="lm", se=F) + facet_grid(lambda~delta) + theme(legend.position="none")

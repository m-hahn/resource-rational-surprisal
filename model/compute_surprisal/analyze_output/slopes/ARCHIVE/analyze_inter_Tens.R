...
library(brms)
if(TRUE){
configurations = unique(data %>% select(deletion_rate, predictability_weight)) %>% filter(deletion_rate*10 == round(deletion_rate*10)) %>% filter(deletion_rate>=0.3, deletion_rate<0.6, predictability_weight>0.25)
...
for(i in (1:nrow(configurations))) {
  ...
  outFileName = paste("slopes/", "FIXED_analyze_inter_Tens.R_VN5_", delta, "_", lambda, ".tsv", sep="")
  if(!file.exists(outFileName)) {
   ...
   for(item_ in unique(data2$Item)) {
      ...
      slope_embBiasTimesDepth = mean(samples[["b_RC.C:EmbBias.C"]] + samples[[paste("r_Item[", item__, ",RC.C:EmbBias.C]", sep="")]])
      slope_compatTimesDepth = mean(samples[["b_RC.C:compatible.C"]] + samples[[paste("r_Item[", item__, ",RC.C:compatible.C]", sep="")]])
      slopes = rbind(slopes, data.frame(Item=c(item_), EmbBias=c(slope_embBias), Compatible=c(slope_compat), EmbBiasTimesDepth=slope_embBiasTimesDepth, CompatibleTimesDepth=slope_compatTimesDepth))

   }
                                                                                                                                 
   write.table(summary(model)$fixed, file=outFileName, sep="\t")          

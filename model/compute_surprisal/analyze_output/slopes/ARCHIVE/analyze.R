...
for(i in (1:nrow(configurations))) {
   ...
   model = brm(SurprisalReweighted ~ RC.C * compatible.C + EmbBias.C*RC.C + EmbBias.C*compatible.C + (1+compatible.C+EmbBias.C|Item) + (1|Noun), data=data2, cores=4, iter=10000)
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze.R_VN5_", delta, "_", lambda, ".tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze.R_VN5_", delta, "_", lambda, ".tsv", sep=""))
   model=NULL
   samples=NULL

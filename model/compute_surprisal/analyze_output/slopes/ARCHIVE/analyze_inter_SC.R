...
for(i in (1:nrow(configurations))) {
   ...
   model = brm(SurprisalReweighted ~ EmbBias.C*compatible.C + (1+compatible.C+EmbBias.C|Item) + (1+compatible.C|Noun), data=data2 %>% filter(RC.C<0), cores=4, iter=5000) # +EmbBias.C*compatible.C
   ...
   write.table(summary(model)$fixed, file=paste("slopes/", "FIXED_analyze_inter_SC.R_VN5_", delta, "_", lambda, ".tsv", sep=""), sep="\t")          
   write.csv(slopes,file=paste("slopes/", "analyze_inter_SC.R_VN5_", delta, "_", lambda, ".tsv", sep=""))
   model=NULL
   samples=NULL

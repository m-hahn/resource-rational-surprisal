...
Datapoints = 251291
data = read.csv("output/analyzeFillers_freq_BNC_Spillover_Averaged_New_R.tsv", sep="\t")
...
meanAIC = 74415.45
plot = ggplot(data %>% group_by(deletion_rate, predictability_weight) %>% summarise(AIC=mean(AIC)-meanAIC) %>% mutate(AIC=pmin(AIC, 50)), aes(x=deletion_rate, y=predictability_weight, fill=AIC)) + geom_tile() + theme_bw() + scale_fill_gradient2() + xlab("Deletion Rate") + ylab("") + guides(fill=guide_legend(title="AIC")) + scale_y_reverse ()
ggsave(plot, file="figures/analyzeFillers_freq_BNC_Spillover_Averaged_New_R.pdf", height=2, width=5)


# Viz for lambda=1
meanAIC = 74415.45
plot = ggplot(data %>% filter(predictability_weight==1) %>% group_by(deletion_rate, predictability_weight) %>% summarise(AIC=mean(AIC)-meanAIC) %>% mutate(AIC=pmin(AIC, 50)), aes(x=deletion_rate, y=predictability_weight, fill=AIC)) + geom_tile() + theme_bw() + scale_fill_gradient2() + xlab("Deletion Rate") + ylab("") + guides(fill=guide_legend(title="AIC")) +  theme(legend.position="bottom")
ggsave(plot, file="figures/analyzeFillers_freq_BNC_Spillover_Averaged_New_R_Lambda1.pdf", height=1.5, width=5)
...
ggsave(plot, file="figures/analyzeFillers_freq_BNC_Spillover_Averaged_New_R_Lambda1_Integer.pdf", height=1.5, width=5)

...
for(i in (1:nrow(configs))) {
   if(TRUE) { #!(ID_  %in% alreadyDone)) {
      ...
      if(nrow(data) > 0) {
         ...
         lmermodel = lmer(ReadingTime.x ~ SurprisalReweighted.x + wordInItem + LogWordFreq.x + LogWordFreq_COCA.R.x + WordLength.x + SurprisalReweighted.y + LogWordFreq.y + LogWordFreq_COCA.R.y + WordLength.y + ReadingTime.y + (1|itemID.x) + (1|subject), data=data %>% filter(RRT.x > 0))

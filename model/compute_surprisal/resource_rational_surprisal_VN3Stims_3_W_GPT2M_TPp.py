...
with open("/u/scr/mhahn/reinforce-logs-both-short/full-logs-retentionProbs/"+__file__+"_"+str(args.load_from_joint), "w") as outFile:
  sys.stdout = outFile
  print(checkpoint["arguments"])
  showAttention("by", POS="IN")
  showAttention("about", POS="IN")
  with open("/u/scr/mhahn/FAIR18/english-test-tagged.txt", "r") as inFile:
     counter = 0
     for line in inFile:
         line = line.strip().split("\t")
         word = line[0].lower()
         if line[0] == line[1]: # punctuation
            continue
         if word in itos:
           if counter % 50 == 0:
                print(counter, file=sys.stderr)
           counter += 1
           showAttention(word, POS=line[1])
           if counter == 200:
              break

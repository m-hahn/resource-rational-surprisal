...
plot = ggplot(data %>% group_by(noun, EmbeddingBias) %>% summarise(MissingVerb=mean(MissingVerb, na.rm=TRUE)), aes(x=EmbeddingBias, y=MissingVerb+0.0)) + geom_point()+ geom_text_repel(aes(label=noun)) + geom_smooth(method="lm")  + theme_bw() + xlab("Embedding Bias") + ylab("Responses with Verb Missing") + ylim(0.1, 0.8)
...
plot = mcmc_areas(model, pars=c("b_Intercept", "b_EmbeddingBias.C"), prob=.95, n_dens=32, adjust=5)

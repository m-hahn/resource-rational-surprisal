...
data$EmbeddingBias = data$True_False_False - data$False_False_False
data$EmbeddingBias.C = data$EmbeddingBias - mean(data$EmbeddingBias, na.rm=TRUE)
...
plot = ggplot(data %>% group_by(noun, EmbeddingBias) %>% summarise(MissingVerb=mean(MissingVerb, na.rm=TRUE)), aes(x=EmbeddingBias, y=MissingVerb)) + geom_point() + geom_smooth(method="lm") + geom_text_repel(aes(label=noun)) + theme_bw() + xlab("Embedding Bias") + ylab("Responses with Verb Missing") + ylim(0.1, 0.8)
ggsave(plot, file="figures/rates_by_conditional.pdf", height=3, width=3)

library(brms)
model = (brm(MissingVerb ~ EmbeddingBias.C + (1+EmbeddingBias.C|workerid) + (1|noun) + (1+EmbeddingBias.C|remainder), data=data, family="bernoulli"))



write.table(summary(model)$fixed, file="output/analysis_Expt3_en_wiki.R_fixed.tsv", sep="\t")
...
samples$EmbeddingBias_wiki = samples$b_EmbeddingBias.C
plot = mcmc_areas(samples, pars=c("Intercept", "EmbeddingBias_wiki"), prob=.95, n_dens=32, adjust=5)
...
print(summary(model))
print(mean(samples$b_EmbeddingBias.C>0))
sink()

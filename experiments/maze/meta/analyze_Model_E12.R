...
sink("analyze_Model_E12_R_AICs.tsv")
cat("deletion_rate", "predictability_weight", "AICRaw", "AIC", "AICFull", "N", sep="\t")
sink()

configs = unique(model %>% select(deletion_rate, predictability_weight) %>% filter(predictability_weight==1))
for(i in (1:nrow(configs))) {
   ...
   sink("analyze_Model_E12_R_AICs.tsv", append=TRUE)
   cat(i, delta, lambda, aicTotallyBare, aicBare, aicFull, sum(!is.na(data_$SurprisalReweighted)), "\n", sep="\t")
   sink()
}

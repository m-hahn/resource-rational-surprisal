...
plot = ggplot(data %>% group_by(wordInItem) %>% summarise(error_rate=mean(correct == "no")), aes(x=wordInItem, y=error_rate)) + geom_line() + theme_bw() + xlab("Position in Sentence") + ylab("Error Rate")
ggsave(plot, file="figures/errors-by-position.pdf", width=4, height=3)
...
model = (brm(correct ~ HasRC.C * compatible.C + HasRC.C * EmbeddingBias.C + HasSC.C * EmbeddingBias.C + compatible.C * EmbeddingBias.C + (1+compatible.C+HasSC.C+HasRC.C|noun) + (1+compatible.C + EmbeddingBias.C + compatible.C * EmbeddingBias.C + HasSC.C + HasSC.C * EmbeddingBias.C + HasRC.C +  HasRC.C * compatible.C + HasRC.C * EmbeddingBias.C|workerid) + (1+compatible.C + EmbeddingBias.C + compatible.C * EmbeddingBias.C + HasSC.C +HasSC.C * EmbeddingBias.C + HasRC.C +  HasRC.C * compatible.C + HasRC.C * EmbeddingBias.C|item), data=data %>% filter(Region == "REGION_3_0"), family="bernoulli"))

sink("output/analyze_Errors.R.txt")
print(summary(model))
sink()

write.table(summary(model)$fixed, file="output/analyze_Errors.R_fixed.tsv", sep="\t")
...
plot = mcmc_areas(samples, pars=c("Depth", "Compatible", "EmbeddingBias", "Embedded"), prob=.95, n_dens=32, adjust=5)
ggsave(plot, file="figures/analyze_Errors.R_posterior-histograms-main_effects.pdf", width=5, height=5)
...
plot = mcmc_areas(samples, pars=c("Depth:Compatible", "Depth:EmbeddingBias", "Embedded:EmbeddingBias", "Compatible:EmbeddingBias"), prob=.95, n_dens=32, adjust=5)
ggsave(plot, file="figures/analyze_Errors.R_posterior-histograms-interactions.pdf", width=5, height=5)

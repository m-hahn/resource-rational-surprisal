...
sink("analyze_Model_IncludingNoTP_SCRC_R_AICs.tsv")
...
for(i in (1:nrow(configs))) {
   ...
   data_ = merge(data %>% filter(HasRC.C>0, Region == "REGION_3_0"), model_ %>% rename(noun=Noun), by=c("noun", "item", "HasSC", "HasRC", "compatible"), all.x=TRUE)
   ...
   sink("analyze_Model_IncludingNoTP_SCRC_R_AICs.tsv", append=TRUE)
   cat(i, delta, lambda, aicTotallyBare, aicBare, aicFull, sum(!is.na(data_$SurprisalReweighted)), "\n", sep="\t")
   sink()
}

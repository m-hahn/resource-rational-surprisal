...
print(cor.test(data__$LogRT, data__$SurprisalReweighted))
...
#plot = ggplot(data__ %>% filter(deletion_rate==0.5, predictability_weight==1), aes(y=LogRT, x=SurprisalReweighted, group=Condition, color=paste(HasSC, HasRC))) + geom_point(aes(shape=Experiment)) + theme_classic() + ylab("Surprisal") + theme(legend.position = "none")
plot = ggplot(data__ %>% filter(Experiment %in% c("E1", "E2"), deletion_rate==0.5, predictability_weight==1), aes(y=LogRT, x=SurprisalReweighted)) + geom_point(aes(shape=Experiment, group=Condition, color=paste(HasSC, HasRC)), alpha=0.5) + theme_classic() + xlab("Surprisal") + theme(legend.position = "none")
ggsave(plot, file="figures/analyze_Model_PlotForExpt12_Joint_ModelHuman_OnlyExpt12_R.pdf", width=3, height=3) # here, full is E1 and dotted is E2

plot = ggplot(data__ %>% filter(Experiment %in% c("E1", "E2"), deletion_rate==0.5, predictability_weight==1), aes(y=LogRT, x=SurprisalReweighted/log(2))) + geom_point(aes(shape=Experiment, group=Condition, color=paste(HasSC, HasRC)), alpha=0.5) + theme_classic() + xlab("Surprisal (bits)") + theme(legend.position = "none")
ggsave(plot, file="figures/analyze_Model_PlotForExpt12_Joint_ModelHuman_OnlyExpt12_R_Bits.pdf", width=3, height=3) # here, full is E1 and dotted is E2




plot = ggplot(data__ %>% filter(Experiment %in% c("E1", "E2"), deletion_rate==0.5, predictability_weight==1), aes(y=rt, x=SurprisalReweighted)) + geom_point(aes(shape=Experiment, group=Condition, color=paste(HasSC, HasRC)), alpha=0.5) + theme_classic() + xlab("Surprisal") + theme(legend.position = "none")
ggsave(plot, file="figures/analyze_Model_PlotForExpt12_Joint_ModelHuman_OnlyExpt12_R_NoLogTransform.pdf", width=3, height=3) # here, full is E1 and dotted is E2

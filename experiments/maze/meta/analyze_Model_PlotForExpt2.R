...
lambda=1
   ...
   model_ = model %>% filter(Region == "V1_0", deletion_rate == delta, predictability_weight == lambda) %>% group_by(Noun, item, Condition) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE))
...
library(ggplot2)
data__ = data_ %>% group_by(condition, noun, Condition, HasRC, HasSC, compatible, EmbeddingBias) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE), LogRT=mean(LogRT, na.rm=TRUE))
...
plot = ggplot(data__, aes(x=LogRT, y=SurprisalReweighted, group=Condition, color=paste(HasSC, HasRC))) + geom_point(aes(shape=compatible)) + theme_classic() # + geom_smooth(method="lm", aes(linetype=compatible))
ggsave(plot, file="figures/analyze_Model_PlotForExpt2_ModelHuman_R.pdf", width=6, height=4)

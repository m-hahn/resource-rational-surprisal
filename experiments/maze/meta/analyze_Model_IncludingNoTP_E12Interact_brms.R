...
data2 = read.csv("../experiment1/Submiterator-master/trials_byWord.tsv", sep="\t") %>% mutate(workerid=workerid+1000, Experiment=1)
data3 = read.csv("../experiment2/Submiterator-master/trials-experiment2.tsv", sep="\t") %>% mutate(workerid=workerid+2000, Experiment=2)
...
#sink("analyze_Model_IncludingNoTP_E1_R_AICs.tsv")
...
#for(i in (1:nrow(configs))) {
   ...
   data_$SurprisalReweighted.C = data_$SurprisalReweighted - mean(data_$SurprisalReweighted, na.rm=TRUE)
   library(brms)
data_$Experiment.C = data_$Experiment - mean(data_$Experiment, na.rm=TRUE)
   modelSurprisal = (brm(LogRT ~ SurprisalReweighted.C*Experiment.C + (1+Experiment.C+SurprisalReweighted.C+SurprisalReweighted.C*Experiment.C|noun) + (1+Experiment.C+SurprisalReweighted.C+SurprisalReweighted.C*Experiment.C|item) + (1+Experiment.C+SurprisalReweighted.C+SurprisalReweighted.C*Experiment.C|workerid), data=data_, cores=4))
 
  sink("output/analyze_Model_IncludingNoTP_E12_Interact_brms.R.txt")
...
#   sink("analyze_Model_IncludingNoTP_E1_R_AICs.tsv", append=TRUE)
#   cat(i, delta, lambda, aicTotallyBare, aicBare, aicFull, sum(!is.na(data_$SurprisalReweighted)), "\n", sep="\t")
#   sink()
#}
#  

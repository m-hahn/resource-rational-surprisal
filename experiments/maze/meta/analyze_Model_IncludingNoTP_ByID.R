...
sink("analyze_Model_IncludingNoTP_ByID_R_AICs.tsv")
cat("ID", "deletion_rate", "predictability_weight", "AICRaw", "AIC", "AICFull", "N", "None\n", sep="\t")
sink()

configs = unique(model %>% select(ID, deletion_rate, predictability_weight))
for(i in (1:nrow(configs))) {
   delta=configs$deletion_rate[[i]]
   lambda=configs$predictability_weight[[i]]
   id = configs$ID[[i]]
   model_ = model %>% filter(Region == "V1_0", ID==id, deletion_rate == delta, predictability_weight == lambda) %>% group_by(Noun, item, Condition) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE), BNCLogWordFreq=mean(BNCLogWordFreq, na.rm=TRUE), COCALogWordFreq=mean(COCALogWordFreq, na.rm=TRUE))
 ...
 if(sum(!is.na(data_$SurprisalReweighted)) > 100) { 
   library(lme4)
   ...
   cat(i, id, delta, lambda, aicTotallyBare, aicBare, aicFull, sum(!is.na(data_$SurprisalReweighted)), "\n", sep="\t")
   sink("analyze_Model_IncludingNoTP_ByID_R_AICs.tsv", append=TRUE)
   cat(i, id, delta, lambda, aicTotallyBare, aicBare, aicFull, sum(!is.na(data_$SurprisalReweighted)), "\n", sep="\t")
   sink()
 }
}
...
# if we really want to do this seriously, we should 
data_$DLT = ifelse(!data_$HasSC, 0, ifelse(!data_$HasRC, 1, 3))



# some are missing: > unique((data_ %>% filter(is.na(SurprisalReweighted)))$item)
# unique((data_ %>% filter(is.na(SurprisalReweighted)))$workerid)


# Also, in the data for Expt2 there is a bug: 238_Critical_Vadv1 and 238_Critical_VAdv1 are the same item

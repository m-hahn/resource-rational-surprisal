...
model_E1 = read.csv("/juice/scr/mhahn/reinforce-logs-both-short/full-logs-tsv-perItem/collect12_NormJudg_Short_Cond_W_GPT2_ByTrial_E1.py.tsv", sep="\t") %>% rename(SurprisalsWithThat=S1, SurprisalsWithoutThat=S2) %>% filter(!(Item %in% c("Mixed_0", "Mixed_10", "Mixed_20", "Mixed_22", "Mixed_27", "Mixed_28", "Mixed_30", "Mixed_32"))) 
...
model = rbind(model_E1)  # model_E0,  

model$item = model$Item


   
   model_ = model %>% filter(Region == "V1_0") %>% group_by(predictability_weight, deletion_rate, Noun, item, Condition) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE))
...
library(ggplot2)
data__ = data_ %>% group_by(deletion_rate, predictability_weight, condition, noun, Condition, HasRC, HasSC, compatible, EmbeddingBias) %>% summarise(SurprisalReweighted=mean(SurprisalReweighted, na.rm=TRUE), LogRT=mean(LogRT, na.rm=TRUE))

plot = ggplot(data__, aes(x=LogRT, y=SurprisalReweighted, group=Condition, color=paste(HasSC, HasRC))) + geom_point(aes(shape=compatible)) + theme_classic() + facet_grid(predictability_weight~deletion_rate) + ylab("Surprisal") + theme(legend.position = "none")
ggsave(plot, file="figures/analyze_ModelIncludingNoTP_PlotForExpt1_Joint_ModelHuman_R.pdf", width=15, height=6)




plot = ggplot(data__ %>% group_by(deletion_rate, predictability_weight) %>% summarise(Correlation = cor(LogRT, SurprisalReweighted)), aes(x=deletion_rate, y=predictability_weight, fill=Correlation)) + geom_tile()  + theme_classic() + scale_fill_gradient2() + scale_y_reverse ()
ggsave(plot, file="figures/analyze_ModelIncludingNoTP_PlotForExpt1_Joint_ModelHuman_Corrs_R.pdf", width=15, height=6)

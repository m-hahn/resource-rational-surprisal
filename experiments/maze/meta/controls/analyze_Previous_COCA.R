...
# Code Embedding Bias
data = data %>% mutate(EmbeddingBias = Conditional_COCA)
data = data %>% mutate(EmbeddingBias.C = Conditional_COCA.C)
...
sink("output/analyze.R_coca.txt")
print(summary(model))
sink()

write.table(summary(model)$fixed, file="output/analyze.R_fixed_coca.tsv", sep="\t")
...
ggsave(plot, file="figures/posterior-histograms-main_effects_coca.pdf", width=5, height=5)
...
ggsave(plot, file="figures/posterior-histograms-interactions_coca.pdf", width=5, height=5)
...
ggsave(plot, file="figures/posterior-histograms_coca.pdf", width=5, height=4)
...
ggsave(plot, file="figures/posterior-histograms-EmbeddingBias_coca.pdf", width=5, height=5)


sink("output/analyze.R_posteriors_coca.txt")
...
ggsave(plot, file="figures/posterior-histograms-RawEffects_EmbeddingBias_coca.pdf", width=5, height=3)

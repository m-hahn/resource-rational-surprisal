...
LogLiks = c()
formula = "LogRT ~ HasRC.C * compatible.C + (1|noun) + (1|workerid) + (1|item)"
...
predictors_names = c("Embedding Bias (Wikipedia)", "Joint (Wikipedia)", "Marginal (Wikipedia)", "Relative Clause Bias", "Homophone Verb", "Deverbal", "Subject Bias", "Embedding Bias (ukWaC)", "Marginal (ukWaC)", "Joint (ukWaC)", "Embedding Bias (COCA)", "Marginal (COCA)", "Joint (COCA)", "Word Length")
for(pred in predictors) {
	formula2 = paste(formula, "+", pred)
	bic = 0.5*deviance(lmer(formula2, data=data %>% filter(HasSC.C>0, Region=="REGION_3_0"), REML=FALSE))
        LogLiks = c(LogLiks, bic)
	cat(pred, "\t", bic, "\n")

}

# Convergence problems: Conditional_ukwac.C, Conditional_COCA.C 

result = data.frame(predictor_code=predictors, LogLik=LogLiks, predictor=predictors_names)
result = result[order(result$LogLik),]
result$predictor = factor(result$predictor, levels=result$predictor)


library(ggplot2)
result$IsEmbBias = grepl("Embedding Bias", result$predictor)
plot = ggplot(result, aes(x=predictor, y=LogLik, color=IsEmbBias)) + geom_point() + theme_bw() + xlab("Predictor") + ylab("Log Likelihood") +   theme(axis.text.x = element_text(angle=90, vjust=.5, hjust=1)) + theme(legend.position = "none")
ggsave(plot, file="analyze_Previous_AIC_Single_R.pdf", height=3, width=4)

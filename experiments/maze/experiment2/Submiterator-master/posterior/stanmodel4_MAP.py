...
parameters["sigma_slope"] = torch.FloatTensor([1.0])
parameters["sigma_intercept"] = torch.FloatTensor([1.0])
...
def model(parameters):
  ...
  log_density += prior_.log_prob(parameters["sigma_intercept"]).sum()
  log_density += prior_.log_prob(parameters["perParticipantEffects_intercept"]/parameters["sigma_intercept"].pow(2)).sum()
  log_density += prior_.log_prob(parameters["sigma_slope"]).sum()
  log_density += prior_.log_prob(parameters["perParticipantEffects_slope"]/parameters["sigma_slope"].pow(2)).sum()
  mix = torch.nn.LogSoftmax()(parameters["mix_logits"])
#  print(predictions.size())
  random_intercept = parameters["perParticipantEffects_intercept"][subjects].unsqueeze(1)
...
#     print("Old and new likelihood", oldLik, float(newLogLikelihood))
   ...
   if iteration % 100 == 0:
       ...
       print(parameters["sigma_slope"])
       print(parameters["sigma_intercept"])

...
with open("trials_byWord.tsv", "w") as outFile:
  print("\t".join(["workerid", "condition", "item", "rt", "correct", "word", "wordInItem", "Region", "RegionFine", "sentence", "Noun", "Continuation", "trial"]), file=outFile)
  with open("trials.tsv", "r") as inFile:
    ...
    for line in inFile:
        ...
        for i in range(len(byWords)):
            print("\t".join([str(x) for x in [workerid, condition, item, byWords[i]["rt"], byWords[i]["correct"], byWords[i]["word"], i, regions[i] if "condition" in condition else "NA", regionsFine[i] if "condition" in condition else "NA", sentence, noun, continuation, line[header["slide_number"]]]]), file=outFile)

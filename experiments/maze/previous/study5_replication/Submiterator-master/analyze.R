...
ggsave(plot, file="figures/posterior-histograms-main_effects.pdf", width=4, height=5)
...
ggsave(plot, file="figures/posterior-histograms-interactions.pdf", width=4, height=5)
...
dataPlot = ggplot(predictions, aes(x=(EmbeddingBias), y=(RTPred), group=paste(HasSC, HasRC, compatible), color=paste(HasSC, HasRC))) + geom_smooth(se=F, method="lm", aes(linetype=compatible)) + geom_point(data=data %>% filter(Region == "REGION_3_0") %>% group_by(HasSC, HasRC, noun, EmbeddingBias, compatible) %>% summarise(rt=mean(exp(LogRT))), aes(x=EmbeddingBias, y=rt, group=paste(HasSC, HasRC, compatible), color=paste(HasSC, HasRC), linetype=compatible), alpha=0.3) +  scale_color_manual(values = c("FALSE FALSE" = "#F8766D", "TRUE FALSE"="#00BA38", "TRUE TRUE"="#619CFF")) + theme_bw() + theme(legend.position="none") + xlab("Embedding Bias") + ylab("Reading Time") + geom_errorbar(data=predictionsPoints, aes(x=EmbeddingBias, ymin=exp(lower), ymax=exp(upper)), width=0.3) + ylim(800, 1700)

             ...
             if RELATION in ["acl:relcl", "ccomp"]: # and childrenAsTrees[0]["dependency"] == "mark": # Object Relatives
                 ...
                 if dominatedBy["posUni"].startswith("N"):
                   ...
                   if headNoun not in headNouns:
                       headNouns[headNoun] = {"ccomp" : 0, "acl:relcl" : 0}
                   headNouns[headNoun][RELATION] = headNouns[headNoun][RELATION] + 1
                   #print(headNouns)
...
with open("results.tsv", "w") as outFile:
   print("\t".join(["Noun", "Count_CCOMP", "Count_Relcl"]), file=outFile)
   for noun, counts in headNouns.items():
      count_ccomp = counts["ccomp"]
      count_relcl = counts["acl:relcl"]
      if count_ccomp == 0:
         continue
      print("\t".join([noun, str(count_ccomp), str(count_relcl)]), file=outFile)

nouns = read.csv("nouns_spanish.tsv", sep="\t")
...
plot = ggplot(nouns, aes(y=Conditional, x=Noun)) + geom_point() + geom_errorbar(aes(ymax=CIUpper, ymin=CILower)) + theme_bw() + theme(axis.text.x=element_text(angle = 90)) + ylab("log P(de que|ART NOUN)")
ggsave(plot, file="../figures/nouns_spanish.pdf", width=5, height=4)
